    ...
    def queryTeamChampionByYear(self, data):
        try:
            try:
                ...
                pass
            self.cur.execute("SELECT campeon, year, COUNT(*) AS conteo FROM lol.lolcito WHERE team = (%s) AND count> 2 GROUP BY (campeon, year)  ORDER BY year DESC, conteo DESC", (data,))
            self.colnames = [desc[0].capitalize() for desc in self.cur.description]
        except Exception, e:
            print str(e)
            print "Can't execute query"
