    ...
    def queryTeamVersus(self, team1, team2, order,offset = 0):
        try:
            try:
                ...
                pass
            if order == "DESC":
                self.cur.execute("SELECT matchHistory AS informacioncompleta, redteamtag AS equiporojo, blueteamtag AS equipoazul, rresult AS resultadoequiporojo, bresult AS resultadoequipoazul, season AS Temporada, year AS Año FROM lol.leagueoflegends WHERE (blueteamtag=(%s) AND redteamtag=(%s) AND SEASON =(%s) AND YEAR=(%s)) OR (blueteamtag=(%s) AND redteamtag=(%s) AND season =(%s) AND year=(%s)) ORDER BY year DESC LIMIT 100 OFFSET (%s)",(team1,team2,season,year,team2,team1,season,year,offset*100))
            else:
                self.cur.execute("SELECT matchHistory AS informacioncompleta, redteamtag AS equiporojo, blueteamtag AS equipoazul, rresult AS resultadoequiporojo, bresult AS resultadoequipoazul season AS Temporada,year AS  Año FROM lol.leagueoflegends WHERE (blueteamtag=(%s) AND redteamtag=(%s) AND SEASON =(%s) AND YEAR=(%s)) OR (blueteamtag=(%s) AND redteamtag=(%s) AND season =(%s) AND year=(%s)) ORDER BY year ASC LIMIT 100 OFFSET (%s)",(team1,team2,season,year,team2,team1,season,year,offset*100))
            self.colnames = [desc[0].capitalize() for desc in self.cur.description]
        except Exception, e:
            print str(e)
            print "Can't execute query"
