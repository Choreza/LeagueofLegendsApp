    ...
    def queryMatchBySeason(self, season, order = "ASC"):
   	try:
		...
		self.cur.fetchall() 
   	        ...
   	        pass
   	    self.cur.execute("SELECT blueteamtag AS BlueTeam, redteamtag AS RedTeam, bresult, rresult, TO_CHAR((gamelength || ' minute')::interval, 'HH24:MI') AS Duración, year AS Año, season AS Temporada FROM lol.leagueoflegends WHERE season = (%s) ORDER BY YEAR %s", (season, order))
   	    self.colnames = [desc[0].capitalize() for desc in self.cur.description]
   	except Exception, e:
   	    print str(e)
   	    print "Can't execute query"
